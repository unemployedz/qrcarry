(() => {
  ...
  // Upload result uses result* IDs while camera result uses scan* IDs.
  const renderResult=(kind,data,error='')=>{
    const ids=kind==='upload'?{box:'uploadResult',type:'resultType',issuer:'resultIssuer',seed:'resultSeed',copy:'resultCopyBtn'}:{box:'scanResult',type:'scanType',issuer:'scanIssuer',seed:'scanSeed',copy:'scanCopyBtn'};
    ...
    const p=parseQRData(data);$(ids.type).textContent=p.type;$(ids.issuer).textContent=p.issuer||'—';$(ids.seed).textContent=p.content||'—';const btn=$(ids.copy);btn.onclick=()=>copy(p.content,btn);
  };
  ...
  async function fileToUsableBlob(file){
    ...
    if(heic&&typeof window.heic2any==='function'){try{const converted=await window.heic2any({blob:file,toType:'image/png'});return Array.isArray(converted)?converted[0]:converted;}catch{}}
    return file;
  }

  async function imageToCanvas(blob){
    const objectUrl=URL.createObjectURL(blob),img=new Image();
    try{await new Promise((resolve,reject)=>{img.onload=resolve;img.onerror=()=>reject(new Error('The browser could not open this image.'));img.src=objectUrl;});const max=2600,scale=Math.min(1,max/Math.max(img.naturalWidth||img.width,img.naturalHeight||img.height));const canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round((img.naturalWidth||img.width)*scale));canvas.height=Math.max(1,Math.round((img.naturalHeight||img.height)*scale));canvas.getContext('2d',{willReadFrequently:true}).drawImage(img,0,0,canvas.width,canvas.height);return canvas;}finally{URL.revokeObjectURL(objectUrl)}
  }

  async function decodeWithZXing(blob){
    if(!window.ZXing?.BrowserQRCodeReader)return null;
    const url=URL.createObjectURL(blob);
    try{const reader=new window.ZXing.BrowserQRCodeReader();const result=await reader.decodeFromImageUrl(url);return result?.getText?.()||result?.text||null;}catch{return null}finally{URL.revokeObjectURL(url)}
  }

  function decodeCanvas(canvas){
    const ctx=canvas.getContext('2d',{willReadFrequently:true}),image=ctx.getImageData(0,0,canvas.width,canvas.height),attempts=[image];
    if(canvas.width>1600){const small=document.createElement('canvas'),ratio=1600/canvas.width;small.width=1600;small.height=Math.round(canvas.height*ratio);small.getContext('2d').drawImage(canvas,0,0,small.width,small.height);attempts.push(small.getContext('2d',{willReadFrequently:true}).getImageData(0,0,small.width,small.height));}
    for(const frame of attempts)for(const inversionAttempts of ['attemptBoth','dontInvert','onlyInvert']){try{const code=window.jsQR?.(frame.data,frame.width,frame.height,{inversionAttempts,greyScaleWeights:{red:.299,green:.587,blue:.114}});if(code?.data)return code.data;}catch{}}
    return null;
  }

  async function decodeOnServer(file){
    const form=new FormData();form.append('file',file,file.name||'qrcode');const response=await fetch('/api/decode',{method:'POST',body:form,headers:{accept:'application/json'}});let payload={};try{payload=await response.json()}catch{}if(!response.ok)throw new Error(payload.error||'The server could not decode this image.');return payload.data||null;
  }
  ...
  async function decodeFile(file){
    ...
    try{
      ...
      $('uploadStatus').textContent='Trying local QR decoders…';
      data=await decodeWithZXing(blob);
      if(!data){const canvas=await imageToCanvas(blob);data=decodeCanvas(canvas)}
      if(data){$('uploadStatus').textContent='Decoded locally.';renderResult('upload',data);return}
      $('uploadStatus').textContent='Local decode missed it. Using server decoder…';
      data=await decodeOnServer(file);
      if(!data)throw new Error('No QR code could be detected. Try the original photo or crop closer to the QR.');
      $('uploadStatus').textContent='Decoded successfully.';renderResult('upload',data);
    }catch(err){console.error(err);$('uploadStatus').textContent='Decode failed.';renderResult('upload','',err?.message||'Unable to decode this image.')}
  }
  ...
  frame.addEventListener('error',()=>showBrowserError(urlInput.value,'The compatibility proxy could not fetch this destination.'));
